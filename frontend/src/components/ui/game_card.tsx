...
function GameCard({ game, onAddToCart}: GameProp) {
  const currentPrice = (game.OriginalPrice * (1 - game.Discount / 100)).toFixed(2);
  const cashback = (parseFloat(currentPrice) * 0.11).toFixed(2);

  return (
    <div className="group relative w-[280px] h-[540px] bg-gametile border-2 border-gametileborder transition-all duration-300 overflow-hidden flex flex-col cursor-pointer shadow-2xl">
      ...
      <div className="absolute bottom-0 left-0 w-full bg-[#1c0533] transition-transform duration-300 ease-out transform translate-y-[125px] group-hover:translate-y-0">
        ...
        <div className="bg-[#110224]/90 backdrop-blur-md py-2 px-4 flex items-center gap-2 border-b border-white/5">
          ...
          <span className="text-[11px] font-bold text-gray-300 uppercase tracking-wider">{game.Store}</span>
        </div>
        ...
        <div className="p-4 flex flex-col h-[340px]">
          ...
          <div className="space-y-2 mt-auto pb-4">
            <button 
              className="w-full bg-[#ffcc00] hover:bg-[#ffdb4d] text-[#1c0533] font-bold py-3 rounded-sm transition-colors text-[16px] tracking-tighter"
              onClick={() => {
                onAddToCart(game);
              }}
            >
